...
enum WeatherURLCenter {
    ...
    func buildURL() -> String {
        ...
        case .forecast(let location, let days):
            guard let lat = location.lat, let lon = location.lon else { return ""}
            return "\(AppConfigManager.secureApiProtocol)://\(AppConfigManager.openWeatherURI)/forecast/daily?lat=\(lat)&lon=\(lon)&cnt=\(days)&appid=\(AppConfigManager.openWeatherKey)&units=metric"
